$(function() {
	var estimateQuotation = $("#estimateQuotation").val();
	var num1 = (estimateQuotation || 0).toString().replace(/(\d)(?=(?:\d{3})+$)/g, '$1,');
	$("#estimateQuotation").val(num1);

	var confirmQuotation = $("#confirmQuotation").val();
	var num2 = (confirmQuotation || 0).toString().replace(/(\d)(?=(?:\d{3})+$)/g, '$1,');
	$("#confirmQuotation").val(num2);
...
$('#addCommentForm .btn').keypress(function(e) {
	if (e.which == 13) {
...
function submitComment(form) {
	$(form).ajaxSubmit({
		url : "projectsManagement/projects/Requirement/addComment.do",
		type : 'post',
