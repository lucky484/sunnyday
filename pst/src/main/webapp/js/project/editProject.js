...
$(function() {
	validator = form.validate({
		...
		rules : {
			projectCode : {
				...
				remote: {
					url: "checkEditProjectCode.do",     //后台处理程序
					type: "post",               //数据发送方式
	...
	if($('input[name="projectNature"]:checked ').val() == 1){
	...
	var projectQuotation = $("#projectQuotation1").val();
	$("#projectQuotation1").val((projectQuotation || 0).toString().replace(/(\d)(?=(?:\d{3})+$)/g, '$1,'));
	$("#projectQuotation").val(projectQuotation);
	var outsourcingQuotation = $("#outsourcingQuotationStr").val();
	$("#outsourcingQuotationStr").val((outsourcingQuotation || 0).toString().replace(/(\d)(?=(?:\d{3})+$)/g, '$1,'));
	$("#outsourcingQuotation").val(outsourcingQuotation);
})
