$(function() {
	$('#addProjectCommentForm').validate({
	...
	$("#projectQuotation").val("￥" + (projectQuotation.toString().replace(/(\d)(?=(?:\d{3})+$)/g, '$1,')));
	$("#outsourcingQuotation").val("￥" + (outsourcingQuotation.toString().replace(/(\d)(?=(?:\d{3})+$)/g, '$1,')));
});

$('#addProjectCommentForm .btn').keypress(function(e) {
	if (e.which == 13) {
		submitComment("#addProjectCommentForm");
		return false;
	}
});
