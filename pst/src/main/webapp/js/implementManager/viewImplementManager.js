$(function() {
	$('#addImplementManagerCommentForm').validate({
...
$('#addImplementManagerCommentForm .btn').keypress(function(e) {
	if (e.which == 13) {
		submitComment("#addImplementManagerCommentForm");
		return false;
	}
});

function getCommentHtml(comment) {
	return '<div class="control-group">' + '评论人：<i class="blue">@' + comment.critic + '&nbsp;&nbsp;</i>' + '评论时间：<i>' + new Date(comment.createTime).Format("yyyy-MM-dd hh:mm:ss") + '</i>' + '<p>' + comment.comment + '</p><hr style="width:45%"/></div>'
}
