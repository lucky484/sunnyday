					...
					<div class="span12">
						...
						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption">
									<i class="icon-reorder"></i> <span class="hidden-480">修改用户</span>
								</div>
							</div>
							<div class="portlet-body form">
								...
								<form id="editUserForm" class="form-horizontal"
									...
									<div class="control-group">
										...
										<div class="controls">
											<input type="text" placeholder="" id="userPassword"
												name="userPassword" value="${um.userPassword}" class="m-wrap span6" />
										</div>
									</div>
									 ...
									 <div class="control-group">
										...
										<div class="controls">
											<input type="text" placeholder="" id="email"
												name="email" value="${um.email}" class="m-wrap span6" />
										</div>
									</div> 
											...
											<select id="roleId" name="roleId"
												...
												<c:forEach items="${roles }" var="role">
													<option value="${role.roleId }"
														<c:if test="${um.roleId==role.roleId }">selected="selected"</c:if>>${role.roleDescription }</option>
												</c:forEach>
											</select>
										</div>
									</div>
									...
									<div class="control-group">
										<label class="control-label">是否邮件通知<span
											class="required">*</span></label>
										<div class="controls">
											<input type="checkbox" placeholder="" id="isNotification_box"
												name="isNotification_box"<c:if test="${um.isNotification==1}"> checked </c:if> style="margin-bottom: -8px;"/>
										</div>
										<input type="hidden" id="isNotification" name="isNotification" value="${um.isNotification}" />
									</div>
