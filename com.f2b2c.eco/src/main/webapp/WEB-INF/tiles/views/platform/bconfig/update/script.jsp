...
<!-- 编辑器源码文件 -->
<spring:url value="/farm/problem/update" var="updateProblemUrl"/>
<spring:url value="/farm/problem/index" var="problemListUrl" />
	 ...
	 UE.Editor.prototype._bkGetActionUrl = UE.Editor.prototype.getActionUrl;  
	  UE.Editor.prototype.getActionUrl = function(action) {  
       ...
       $("#publish").on("click",function() {
    	...
    	//问题回答
    	var detail = $("#response").val();
    	
    	// 验证
		if(title == undefined || title == null || title == ""){
			$.notify("error", "请输入问题名称");
			$("#publish").attr("disabled",false)
			return;
		}
		if(detail == undefined || detail == null || detail == ""){
			$.notify("error", "请输入问题回答");
			$("#publish").attr("disabled",false)
			return;
		} 
    	 ...
    	 $.ajax({
    			...
    			"success" : function(data) {
    				if(data.status == 400){
    					...
    					window.location.href="${problemListUrl}";
    				}
    			},
				error:function(data){
					$("#publish").attr("disabled",false);
