...
<body style="overflow-x: hidden;">
	<div id="window" style="overflow-x:hidden;">
		<div>
		<c:if test="${type==0 }">
		<form id="FarmOrderList" action="<c:url value="/wxpay/pay-1/pre_share.action"/>" method="post" onsubmit="return checkForm();">
			<br/><input name="openid" value="${openid }" type="hidden" />
			<table width="100%">
				<tr>
						...
						<div class="font_16" id="" >
							<div class="">
								<strong class="tb-promo-price">
									<em class="tb-rmb text-decoration">¥108</em>
									<em class="tb-rmb-margin">¥</em>
									<input class="tb-rmb" id="unitPrice" name="unitPrice" readonly="readonly" value="93" style="border: 0; width: 12%;" />
			...
			<div class=" bottom_order2" >
				...
				<div class="clear bottom_order2" style="background-color: #fff;border-top: 1px solid #ccc;">
					<div class="float_l font_16" style="margin-left:30px;margin-top:6px;">
						...
						<c:if test="${type==0 }"><!-- 苹果 -->
						<input id="total" name="total" type="hidden" value="93"/>&nbsp;
						<input type="hidden" class="" id="quan" value="0" style="border: 0; width: 4%;" />
						<span id="total_read" class="font_16">93</span>
						</c:if>
					</div>
		...
		<form id="FarmOrderList" action="<c:url value="/wxpay/pay-1/pre_share.action"/>" method="post" onsubmit="return checkForm();">
			...
			<div class="magrin_t3">
				<div class="float_l margin_l1">
					<c:if test="${type==1 }">
						<img src="<c:url value="/resources/images/farmOrder/apple/christmas.jpeg"/>" style="width:80px;height: 80px;"/>
					</c:if>
					<c:if test="${type==2 }">
						<img src="<c:url value="/resources/images/farmOrder/apple/jingpin.jpeg"/>" style="width:80px;height: 80px;"/>
					</c:if>
					<c:if test="${type==3 }">
				...
				<div class="float_l" style="width:70%;">
					...
					<c:if test="${type==1 }">
						<input class="font_16 padding_l1" id="produceName" name="produceName" style="font-weight: 700;border: 0; width:600px" value="阿克苏苹果 圣诞组合" readonly="true" />
							...
							<div class="">
								<c:if test="${type==0 }">
									<strong class="tb-promo-price">
										<em class="tb-rmb text-decoration">¥108</em>
										<em class="tb-rmb-margin">¥</em>
										<input class="tb-rmb" id="unitPrice" name="unitPrice" readonly="readonly" value="103" style="border: 0; width: 12%;" />
										<font style="font-weight: 100;">8.5公斤/箱</font>
									</strong>
									<span id="" class="tb-promo-type" style="font-size: 12px;">产地直销</span>
								</c:if>
								<c:if test="${type==1 }"><!-- 圣诞组合苹果 -->
									<strong class="tb-promo-price">
										<em class="tb-rmb-margin">¥</em>
										<input class="tb-rmb" id="unitPrice" name="unitPrice" readonly="readonly" value="45.9" style="border: 0; width: 18%;" />
									</strong>
								</c:if>
								<c:if test="${type==2 }"><!-- 精装苹果 -->
									<strong class="tb-promo-price">
										<em class="tb-rmb-margin">¥</em>
										<input class="tb-rmb" id="unitPrice" name="unitPrice" readonly="readonly" value="29.9" style="border: 0; width: 18%;" />
									</strong>
								</c:if>
								<c:if test="${type==3 }"><!-- 2.5公斤/箱苹果 -->
									<strong class="tb-promo-price">
										<em class="tb-rmb-margin">¥</em>
										<input class="tb-rmb" id="unitPrice" name="unitPrice" readonly="readonly" value="49.9" style="border: 0; width: 18%;" />
									</strong>
								</c:if>
							</div>
							<div class="tb-promo-item-ft"></div>
				...
				<div class="clear bottom_order2" style="background-color: #fff;border-top: 1px solid #ccc;">
					<div class="float_l font_16" style="margin-left:30px;margin-top:6px;">
						...
						<c:if test="${type==0 && status != 1}"><!-- 8.5kg苹果 ,无抵用券-->
							<input id="total" name="total" type="hidden" value="103"/>&nbsp;
							<span id="total_read" class="font_16">103</span>
						</c:if>
						<c:if test="${type==0 && status == 1}"><!-- 8.5kg苹果,有抵用券  -->
							<input id="total" name="total" type="hidden" value="98"/>&nbsp;
							<span id="total_read" class="font_16">98</span>
						</c:if>
						<c:if test="${type==1}"><!-- 圣诞组合苹果 -->
							<input id="total" name="total" type="hidden" value="45.9"/>&nbsp;
							<span id="total_read" class="font_16">45.9</span>
						</c:if>
						<c:if test="${type==2}"><!-- 精装苹果 -->
							<input id="total" name="total" type="hidden" value="29.9"/>&nbsp;
							<span id="total_read" class="font_16">29.9</span>
						</c:if>
						<c:if test="${type==3}"><!-- 2.5公斤/箱苹果 -->
							<input id="total" name="total" type="hidden" value="49.9"/>&nbsp;
							<span id="total_read" class="font_16">49.9</span>
						</c:if>
					</div>
					<input type="submit" class="float_r" id="submit" style="width:50%; border-radius: 0px;" ></input>
				</div>
			...
			$.ready(function() {
				...
				showCityPickerButton.addEventListener('tap', function(event) {
					cityPicker3.show(function(items) {
						showCityPickerButton.innerText =(items[0] || {}).text + " " + (items[1] || {}).text + " " + (items[2] || {}).text;//原cityPicker3
						doc.getElementById("province").value = (items[0] || {}).text;
						doc.getElementById("city").value = (items[1] || {}).text;
						doc.getElementById("town").value = (items[2] || {}).text;
						//江浙沪包邮，偏远不予下单，其他20￥/箱
						if((items[0] || {}).text=="上海市" || (items[0] || {}).text=="江苏省" || (items[0] || {}).text=="浙江省"){
							doc.getElementById("logistics").value=0;
							single_logistics = 0;
							total_logistics = 0;
						}else {
							//其他地区  总运费
							doc.getElementById("logistics").value=20*doc.getElementById("weight").value;
							single_logistics = 20; //其他地区运费20
							total_logistics = doc.getElementById("logistics").value;//其他地区  总运费
						}
						doc.getElementById("total").value=parseInt(doc.getElementById("logistics").value) + parseInt(doc.getElementById("unitPrice").value) * parseInt(doc.getElementById("weight").value);
						doc.getElementById("total_read").innerText=doc.getElementById("total").value;
					});
				}, false);//返回 false 可以阻止选择框的关闭
